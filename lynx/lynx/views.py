...
def generate_number_of_services_report(request):
    ...
    """
    if request.method == 'POST':
        form = lfo.NumberOfServicesForm(request.POST)
        if not form.is_valid():
            messages.error(request, "Invalid date format. Please correct the form fields.")
            return redirect('lynx:reports')

        start_date = form.cleaned_data['start_date']
        end_date = form.cleaned_data['end_date']
        include_event_dates = form.cleaned_data.get('include_event_dates')

        try:
            report_data = lunos.get_number_of_services(start_date, end_date)
        except Exception as e:
            logger.exception("Error generating number of services report")
    ...
    form = lfo.NumberOfServicesForm(initial={
        'start_date': current_grant_year_start_date,
        'end_date': date.today(),
...
class AuthorizationUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = 'Edit Authorization'
        return context
...
class ProgressReportDeleteView(UserPassesTestMixin, DeleteView):
    ...
    def get_success_url(self):
        ...
        client_id = self.kwargs.get('client_id') or self.kwargs.get('contact_id')
        if not client_id:
            auth = getattr(self.object, 'authorization', None) if getattr(self, 'object', None) else None
            client_id = getattr(auth, 'contact_id', None) if auth else None
...
class AuthorizationDeleteView(UserPassesTestMixin, DeleteView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = 'Delete Authorization?'
        return context
